...
$query = "SELECT * FROM invoice_out WHERE 1";
...
if (!empty($search)) {
    $query .= " AND (invoice_number LIKE '%$search%' OR target LIKE '%$search%' OR reason LIKE '%$search%')";
}
...
$total_rows_query = "SELECT COUNT(*) AS total FROM invoice_out WHERE 1";
if ($start_date && $end_date) {
...
if (!empty($search)) {
    $total_rows_query .= " AND (invoice_number LIKE '%$search%' OR target LIKE '%$search%' OR reason LIKE '%$search%')";
}
...
<body>
    ...
    <form method="GET" action="">
        ...
        <input type="text" name="search" value="<?php echo isset($_GET['search']) ? $_GET['search'] : ''; ?>" placeholder="Enter search term">


        <button type="submit">Filter</button>
    </form>
    <div class="btn-container">
        <a href="InvoiceOut-Adding.php" class="btn-invoice">Add New Payment</a>
    </div>
    <!-- Payments Table -->
    <table>
        <thead>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <?php if (!empty($row['invoice_file'])): ?>
                            ...
                            <a href="view_file.php?id=<?php echo urlencode($row['id']); ?>&table=invoice_out&btn=view" target="_blank">
                                <button type="button">View File</button>
                            </a>
                            <a href="view_file.php?id=<?php echo urlencode($row['id']); ?>&table=invoice_out&btn=download" target="_blank">
                                <button type="button">Download File</button>
                            </a>

                            <form action="download_file.php?id=<?php echo urlencode($row['id']); ?>&table=invoice_out" method="POST" enctype="multipart/form-data">
                    ...
                    <td id="UDform">
                        ...
                        <form action="CRUD-out.php" method="POST" class="UDform" onsubmit="return confirm('Are you sure?')">
    ...
    <div class="pagination">
        ...
        <?php for ($i = 1; $i <= $total_pages; $i++): ?>
            <a href="?page=<?php echo $i; ?>&search=<?php echo urlencode($search); ?>" <?php if ($page == $i) echo 'class="active"'; ?>>
                <?php echo $i; ?>
            </a>
        <?php endfor; ?>
    ...
    <div class="modal fade popupForm" id="modifying" tabindex="-1" role="dialog" aria-labelledby="editModalLabel" aria-hidden="true">
        ...
        <form id="editInvoiceForm" method="POST" action="CRUD-out.php">
        ...
        <script>
            ...
            function fetchProductDetails() {
                ...
                xhr.send("id=" + encodeURIComponent(invoiceId) + "&table=invoice_out");
            }
        </script>
    </div>
